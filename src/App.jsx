...
const initVendors = [
  {id:"v1",name:"O'Leary Group",role:"Construction Consultant",contact:"Samara Petigrow (samara@olearygroup.com), Conor O'Leary",status:"active",risk:"low",lastUpdate:"Mar 2026",contract:"Signed Jan 25, 2026 (DocuSign). Weekly fee, Dec 2025 through opening + 6 weeks. Est. $301-329K total.",notes:"Manages the Smartsheet construction timeline ('Hero's Journey Flatiron Schedule'). Andrey has view-only access. Key interface between HJ and all construction partners. Paid to date: Inv #1723 $9,320 (Nov 25) + #1727 $14,776 (Jan 26) + #1753 $21,000 (Feb 26) + #1754 $20,000 (Mar 26) = $65,096 total.",deliverables:["Smartsheet timeline management","Construction oversight","Vendor coordination","Contract/procurement management"],nextAction:"Request automated Smartsheet email reports (daily/weekly)",dueDate:"2026-03-20",budget:329000,spent:65096},
        ...
        {tab==="overview"&&(
          <div className="fi">
            ...
            <div className="g4" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:16}}>
              {[{label:"GC Budget LOW",val:fmt$(totalLow),sub:`HIGH: ${fmt$(totalHigh)}`,c:"#7C3AED",bg:"linear-gradient(135deg,#7C3AED,#A78BFA)",go:"budget"},{label:"Spent to Date",val:fmt$(totalSpent),sub:`committed: ${fmt$(totalCommitted)}`,c:"#EF4444",bg:"linear-gradient(135deg,#EF4444,#F87171)",go:"budget"},{label:"Tasks Complete",val:`${doneTasks.length}/${allTasks.length}`,sub:`${overdueCount} overdue`,c:overdueCount?"#F97316":"#22C55E",bg:overdueCount?"linear-gradient(135deg,#F97316,#FB923C)":"linear-gradient(135deg,#22C55E,#4ADE80)",go:"timeline"},{label:"Open Issues",val:openIssues.length,sub:`${critIssues.length} critical`,c:critIssues.length?"#EF4444":"#64748B",bg:critIssues.length?"linear-gradient(135deg,#EF4444,#F87171)":"linear-gradient(135deg,#64748B,#94A3B8)",go:"issues"}].map((s,i)=>(
                <div key={i} onClick={()=>setTab(s.go)} style={{...card,background:s.bg,color:"#fff",border:"none",cursor:"pointer",transition:"transform .15s, box-shadow .15s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 20px rgba(0,0,0,0.15)"}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow=""}}>
        ...
        {tab==="access"&&isAdmin&&(
          <div className="fi">
            ...
            <div style={{...card,marginTop:14,background:"#FFFBEB",border:"1px solid #FDE68A"}}>
              <div style={{fontSize:12,fontWeight:700,color:"#92400E",marginBottom:8}}>How it works</div>
              <div style={{fontSize:12,color:"#92400E",lineHeight:1.7}}>
                1. Add an email address above to grant access.<br/>
                2. Share the Command Center URL with the team member.<br/>
                3. On their first visit, they'll create their own password.<br/>
                4. After that, they log in with email + password.<br/>
                5. Only you (admin) can add or remove users.
              </div>
            </div>
          </div>
        )}
